...
.claude/
  skills -> ../.agents/skills   링크. git 미포함, setup 스크립트로 생성
  commands/                슬래시 커맨드
  agents/                  서브에이전트
scripts/setup-harness.*    링크 생성 스크립트
```
...
| 도구 | 규칙 | 스킬 |
|---|---|---|
| Claude Code | `CLAUDE.md` → `@AGENTS.md` import | `.claude/skills` 링크 필요 |
| Cursor · Antigravity | `AGENTS.md` 직접 읽음 | `.cursor/skills` 링크를 같은 방식으로 걸면 공유 가능 |

Claude Code는 `AGENTS.md`도 `.agents/skills/`도 읽지 않는다. 그래서 `CLAUDE.md`가 `@AGENTS.md`를 import하고, `.claude/skills`가 `.agents/skills`를 가리키는 링크로 연결한다. **clone 직후 `scripts/setup-harness.sh`(또는 `.ps1`)를 한 번 실행해야 스킬이 로드된다.**
