...
- TASK 형식: `TASK/task1/GitHub_Project_TASK_템플릿.md`
...
```text
TASK/task1/{TYPE}-{NNN}_{기능-요약}.md
```
...
예시 파일명은 `TASK/task2/API-001_계산-요청응답-계약.md`이다. GitHub Issue 번호는 실제 등록 후 `Dependencies & Interactions`에 별도로 기록한다.

## 6. 결론

권장 추출·실행 순서는 `계약·데이터 → 실패하는 단위·계약 테스트 → 로직 → 비기능·E2E → UX 설계 → Frontend 구현`이다. 이 순서를 따르면 AI가 구현 전에 자동 채점 기준을 확보하고, 프론트엔드는 승인된 UX와 Mock 계약을 기반으로 작업하며, 후행 구현이 데이터 구조·상태 의미·카피를 다시 추측하는 일을 줄일 수 있다. 실제 구현은 수직 경로마다 실패 테스트를 먼저 고정한 뒤 Logic·UX·Frontend·NFR을 병렬 진행한다.
