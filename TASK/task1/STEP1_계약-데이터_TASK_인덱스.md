...
CardFit의 백엔드·프론트엔드·테스트가 공유할 데이터와 통신 계약을 먼저 고정하기 위한 Step 1 실행 목록이다. 총 11개의 풀버전 TASK를 의존 순서대로 작성하며, M1 필수 계약과 M2 후속 계약을 구분한다. 읽기 ViewModel과 제품 이벤트 schema는 Logic 구현에 역의존하지 않도록 SPEC-001·002가 선행 소유한다.
...
| 9 | `SPEC-001 읽기 ViewModel·오류·상태 계약` | M1/M2 | DATA-001~003, API-001~005 | QUERY-001~004, UX/UI, MOCK-001 |
| 10 | `SPEC-002 제품·Guardrail 이벤트 계약` | M1/M2 | DATA-001~003, KPI·Guardrail 산식 | COMMAND-010, QUERY-003·004, NFR-006, MOCK-001 |
| 11 | `MOCK-001 비식별 Fixture·Mock 응답 규격` | M1/M2 | 모든 적용 대상 DATA·API·SPEC 계약 | 프론트엔드 병렬 개발, 계약·E2E 테스트 |

## 상호작용 기준

- DATA 변경은 연결 API DTO, Mock Fixture, migration 및 테스트 데이터에 함께 전파한다.
- API와 SPEC ViewModel 변경은 소비 Query·UI·Route Handler·Server Action·계약 테스트에 함께 전파한다.
...
- SRS 8.10의 모든 Action·Route 및 Platform Adapter 메서드가 API TASK에 매핑되고, 읽기 출력 계약은 SPEC-001에 매핑된다.
- KPI·Guardrail 이벤트 이름·payload·멱등성·개인정보 경계는 SPEC-002에 매핑된다.
...
Step 1은 계산 구현보다 먼저 데이터 의미와 통신 경계를 확정한다. DATA 계약과 API-001~005를 먼저 고정하고 SPEC-001·002의 읽기 모델·이벤트 계약을 확정한 뒤 MOCK-001로 모든 정상·실패 상태를 완결한다. 프론트엔드는 MOCK-001 전체 완료 전에도 확정된 M1 계약의 Fixture부터 순차적으로 사용할 수 있다.
...
### 2026-08-26 — ViewModel·이벤트 계약을 Step 1 SPEC으로 분리

- 결정: Step 1에 SPEC-001·002를 추가해 읽기 ViewModel과 제품·Guardrail 이벤트 계약을 Logic보다 먼저 고정한다.
- 근거: QUERY와 COMMAND가 계약까지 소유하면 MOCK-001이 Step 2에 역의존해 Contract 단계를 독립적으로 완료할 수 없다.
- 영향: Step 1 TASK는 9개에서 11개로 늘어나며, QUERY-001~004와 COMMAND-010은 각 SPEC을 구현하는 Logic TASK가 된다.
