...
**근거 문서:** `TASK/task1/01_전체_TASK_목록_및_기준선.md`(54건) · `TASK/task2/*.md`(TASK별 이슈 명세 54건)

> ⚙️ 이 문서는 `TASK/task2` 54개 TASK 본문의 `Dependencies & Interactions`를 직접 대조해 다시 계산한 실행계획이다. 기존 `02_전체_TASK_실행전략_및_의존성.md`는 총계 표기만 54로 갱신되고 Wave·DAG·Gantt 본문은 52개 기준으로 남아 있는 등 정본(task2)과 어긋나는 부분이 발견되어 이 문서로 전면 대체했다(경위는 문서 끝 Decision Log 참고).
...
| 문서 | 답하는 질문 |
| --- | --- |
| SRS (`SRS-Drafts/SRS_CardFit_v1.6_GPT-5.6-SOL.md`) | **무엇을** 만드는가 |
| PRD (`PRD/PRD_CardFit_v1.3.md`) | **왜** 만드는가 |
| TASK 목록 (`01_전체_TASK_목록_및_기준선.md`) | **무엇을 쪼개** 만드는가 |
| `TASK/task2/*.md` | **각 조각을 어떻게** 끝내는가 (AC·DoD) |
| **본 문서** | **어떤 순서로, 누가, 언제** 만드는가 |

실무자는 본 문서에서 자기 트랙과 착수 시점을 확인하고, 실제 작업은 `TASK/task2/<TASK-ID>.md`의 Task Breakdown과 Acceptance Criteria를 따른다.
...
| 5 | **게이트를 통과해야 다음으로 간다** — `SEC-001`·`INFRA-001`은 M1 배포를 막는 독립 Gate다 | `TASK/task2/SEC-001`·`INFRA-001` | 검증되지 않은 보안·배포 상태로 릴리스한다 |
...
이 6건은 `TASK/task1/04_정책_결정_로그.md`의 Decision Log로 추적하며, 개발 TASK 수(54개)와 분리해 관리한다.
...
상세 통과 기준은 `01` §8과 `TASK/task2`의 개별 `Verification Gates`를 정본으로 한다.
...
TASK가 추가·삭제되거나 의존성이 바뀌면 이 문서 §2·§3을 다시 계산한다. 단일 원천은 `TASK/task2/*.md`의 `Dependencies & Interactions`이며, 이 문서는 그 값을 옮긴 결과물이다. `TASK/task1/01_전체_TASK_목록_및_기준선.md`의 "선행 태스크" 열은 이 문서와 항상 같은 값을 가져야 한다.
...
- 결정: 기존 `02_전체_TASK_실행전략_및_의존성.md`를 삭제하고, `TASK/task2` 54개 TASK 본문의 `Dependencies & Interactions`를 직접 대조해 재계산한 이 문서로 대체한다.
- 근거: 기존 문서는 총계 표기만 54로 갱신됐고 Wave·DAG·Gantt 본문은 52개 기준으로 남아 있었으며, 대조 감사 결과 `TEST-006/007`·`COMMAND-010`·`UI-004·006`·`UX-003`에서 정본(task2)과 요약본(01·구 02) 간 의존성 불일치가 확인됐다.
- 수행: `TASK/task2`에서 `API-003·006`(존재하지 않는 `API-006`) 3건, `UX-005`(제거된 ID) 1건을 제거하고 `COMMAND-010`에 누락됐던 `SPEC-002` 의존을 추가한 뒤, 수정된 정본을 기준으로 DAG 레벨·임계 경로·병목·자원 제약 일정을 다시 계산했다.
- 영향: `01`의 "선행 태스크" 열과 이 문서의 의존성 데이터가 일치함을 확인했다(§5.3). 향후 TASK 의존성이 바뀌면 `task2` 본문을 먼저 고치고 이 문서를 재계산한다. `03_병렬실행_Gantt_로드맵.md`는 이 문서의 요약본 역할로 유지하며, 다음 갱신 시 이 문서 기준으로 재동기화한다.
