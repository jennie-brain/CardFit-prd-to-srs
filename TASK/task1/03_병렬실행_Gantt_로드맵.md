...
이 문서는 [전체 TASK 구현 실행계획](02_전체_TASK_실행전략_및_의존성.md)의 상세 DAG를 실행 그룹 단위로 압축한 한눈보기용 로드맵이다. 막대 하나는 새 TASK가 아니라, `TASK/task2`의 상세 TASK 여러 개를 같은 의존성 구간에서 병렬로 실행하는 **작업 묶음**이다.
...
- [전체 TASK 구현 실행계획](02_전체_TASK_실행전략_및_의존성.md)
