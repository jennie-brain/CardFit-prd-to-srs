...
| 순서 | TASK | 담당 요구사항 | Depends on | Blocks |
|---:|---|---|---|---|
| 1 | QUERY-001 온보딩 ViewModel·컨텍스트·초기값 조회 | 002, 008, 009 | DATA-001, API-001, COMMAND-001·002 | 입력 UI |
| 2 | QUERY-002 계산 결과 ViewModel·근거·단계 제안 조회 | 005, 007, 011 | DATA-002, API-003, COMMAND-003 | 결과·근거 UI |
| 3 | QUERY-003 Outcome ViewModel·이행 상태·품질 지표 조회 | 010 | DATA-003, API-004, COMMAND-004~006·010 | 이행 UI·KPI |
| 4 | QUERY-004 운영 ViewModel·Rule·Health·Guardrail 조회 | 009, NF-006·007·009 | API-005, COMMAND-007·010 | 관리자 UI·운영 점검 |

## CQRS 경계 원칙

- Query는 DB·외부 상태를 변경하지 않고 자기 화면 소비자가 사용할 versioned ViewModel 계약을 함께 소유한다. 선택적 AI 설명 생성처럼 비용·캐시 쓰기가 발생하는 작업은 API-003 계약 안에서 별도 Command 성격으로 다룬다.
