...
| 순서 | TASK | 담당 요구사항 | Depends on | Blocks |
|---:|---|---|---|---|
| 1 | QUERY-001 온보딩 컨텍스트·초기값 조회 | 002, 008, 009 | DATA-001, API-001, COMMAND-001·002 | 입력 UI |
| 2 | QUERY-002 계산 결과·근거·단계 제안 조회 | 005, 007, 011 | DATA-002, API-003, COMMAND-003 | 결과·근거 UI |
| 3 | QUERY-003 이행 상태·품질 지표 조회 | 010 | DATA-003, COMMAND-004~006 | 이행 UI·KPI |
| 4 | QUERY-004 Rule·Health·Guardrail 조회 | 009, NF-006·007·009 | API-005, COMMAND-007 | 관리자 UI·운영 점검 |
...
Step 2는 8개 Command와 4개 Query로 구성한다. 다음 단계의 TEST는 이 문서들의 정상·실패 AC를 실행 코드로 증명하고, 테스트 TASK ID를 각 로직 TASK의 DoD에 연결한다.
