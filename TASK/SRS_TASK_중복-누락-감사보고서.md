...
최신 SRS의 요구사항과 TASK 53개를 식별자·책임·의존성 기준으로 대조했다. 감사 결과는 **TASK ID 중복 0건, SRS 요구사항 78개 중 활성 요구사항 미매핑 0건, 실제 기능 중복 0건, Deferred 1건, 외부 결정 Blocker 6종**이다. 콘텐츠 정책 계약·검수와 이벤트 계약·기록은 각각 동일 변경 단위이므로 COMMAND-008·010으로 병합했고, UX와 Frontend의 책임 경계는 유지했다.
...
- 식별자 검사: 파일명 기준 53개, 고유 ID 53개, 중복 ID 0개
...
| REQ-FUNC-009 | COMMAND-008 정책 계약, API-006 | COMMAND-008 검수, QUERY-001 | TEST-003·006 | UX-001·002·004·005, UI-002·006·007 | 승인 문구 조건부 |
...
| REQ-GR-005 | COMMAND-007·010, NFR-005 | TEST-002·006, UI-004·009 | 충족 |
...
| REQ-SEC-002 | API-001~006, COMMAND-001~010, TEST-005 | 충족 |
...
| SRS ID | 주요 TASK | 판정 |
|---|---|---|
| REQ-ARCH-001 | NFR-003, API-001~006, COMMAND-010 | 충족 |
| REQ-ARCH-002 | DATA-001~003, API-001, NFR-004 | 충족 |
| REQ-ARCH-003 | API-002, COMMAND-001·004·007 | 충족 |
| REQ-ARCH-004 | API-003~006, COMMAND-009·010 | 충족 |
...
| 후보 | 중복 여부 | 책임 경계 |
|---|:---:|---|
| DATA-001~003 vs API-001~006 | 아님 | DATA는 관계형 저장 구조, API는 프로세스 간 입출력 계약 |
...
SRS 요구사항 78개 중 활성 요구사항 77개는 TASK 53개에 모두 매핑됐고, REQ-METRIC-007은 SRS 결정에 따라 Deferred로 유지된다. ID와 기능 책임 기준의 중복은 없으며, 같은 기능명이 여러 문서에 나타나는 경우 Contract·Logic·Test·NFR·UX·Frontend의 서로 다른 완료 산출물을 의미한다. 콘텐츠 정책 계약·검수와 이벤트 계약·기록은 각각 동일 변경 단위로 병합했으며 UX 설계와 Frontend 구현은 별도 TASK로 유지했다. 남은 항목은 새 TASK 누락이 아니라 구현 값을 결정하는 외부 Blocker이며, 해당 TASK를 `Blocked` 상태로 관리해야 한다.
