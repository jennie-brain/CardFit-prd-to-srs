...
| MOCK-001 | Mock Platform | 비식별 Fixture·Mock 응답 규격 | 1.2.1, 3.2~3.4, 9.4~9.5 | DATA-001~003, API-001~006, COMMAND-010 이벤트 계약 | H |
...
| COMMAND-010 | Product Analytics | 제품·퍼널·Guardrail 이벤트 계약·기록 | 5.4, 9.1~9.3 | DATA-001~003, KPI·Guardrail 산식 | M |
...
| TEST-003 | Evidence Test | M1 근거·스코프·단계 정렬 + M2 AI fallback 테스트 | TC-FUNC-007·009·011 | M1: QUERY-002, COMMAND-008 / M2: COMMAND-009 | M |
...
| Task ID | Epic (도메인) | Feature (기능명) | 관련 SRS 섹션 | 선행 태스크 (Dependencies) | 복잡도 |
|---|---|---|---|---|:---:|
| UX-001 | UX Foundation | 공통 정보구조·상태 언어·접근성 설계 | 3.1, 9.5, REQ-UI-001~005 | COMMAND-008 정책 계약, API-006 | M |
...
| UX-004 | Evidence UX | 배분·근거·AI 설명 정보 계층 | REQ-FUNC-006·007, UX-V-004·006 | UX-001·003, API-003·006, COMMAND-008 정책 계약 | M |
| UX-005 | Selection UX | 조합 선택·외부 이동 경계 설계 | REQ-FUNC-009·010, ADR-002 | UX-003·004, COMMAND-008 정책 계약 | M |
| UX-006 | Outcome UX | 이행 자기보고·관측 신뢰도 설계 | REQ-FUNC-010, UX-V-007·008 | UX-001·005, DATA-003, API-006 | H |
| UX-007 | Admin UX | 관리자 Guardrail 의사결정 설계 | REQ-NF-004~009 | UX-001, API-005·006, COMMAND-010 이벤트 계약, NFR-002~006 | M |
...
|---|---:|---|---|
| 데이터·계약 | 10 | DATA-001·002, API-001~003·005·006, MOCK-001 M1 | DATA-003, API-004, MOCK M2 |
...
CardFit 개발 범위는 53개의 Epic·Feature TASK로 분해된다. UX 설계는 정보 구조·흐름·상태·카피를 결정하고 Frontend TASK는 승인 명세를 코드로 구현한다. M1은 사용자가 미래지출을 입력하고 세 시나리오의 유지·변경 결론, 차액, 배분과 근거를 이해한 뒤 조합안을 선택하는 경험을 완성하는 데 집중한다. M2의 AI 설명·초기값·자동 이행 관측은 M1 핵심 경로를 차단하지 않으며, M3 실제 통합은 내부 계약과 컴플라이언스 승인 이후 별도 Gate로 관리한다.
