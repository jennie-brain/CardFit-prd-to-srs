...
- `CalculationResultView`의 versioned schema와 empty·partial·stale·unavailable·error 상태 계약
...
- [ ] 내부 DB enum·오류를 화면용 domain code로 변환하고 민감 원본을 제외한다.
...
- Server Component와 Route Handler는 같은 `CalculationResultView` DTO를 사용한다.
...
- [ ] `CalculationResultView` 정상·부분·실패 예시와 contract assertion이 고정됐다.

## Dependencies & Interactions
- Depends on: DATA-002, API-003, TEST-002·003 실패 기준선, COMMAND-003
