...
- Reads: DATA-001과 외부 Identity·Consent·Card·Spend·Catalog 계약
- Writes: Adapter port, 공통 envelope와 오류 schema
- Side Effects: 계약 작성은 `None`. 구현체의 외부 호출은 후행 Logic이 담당한다.
- Transaction Boundary: None. 외부 조회 계약이다.
- Idempotency: 동일 조회 기준시각·요청키의 의미를 구현체 간 동일하게 정의한다.
- Retry Policy: timeout·429·5xx만 제한적으로 재시도하고 동의·인가 오류는 재시도하지 않는다.
...
- Test Gate: TEST-001·005와 MOCK-001의 공통 Adapter 계약 검사가 통과해야 한다.
- NFR Gate: NFR-004·006의 마스킹·호출비용·오조회 기준을 충족해야 한다.
- Evidence Location: port schema, 오류 코드표와 Mock/Production 공통 계약 테스트 결과
...
- Depends on: DATA-001
- Blocks: API-003, MOCK-001, COMMAND-002, QUERY-001·002, TEST-001
...
- 결정: API-001에 Execution Contract와 Verification Gates를 추가한다.
- 근거: Adapter 교체 가능성과 재시도·오류 의미를 구현 전에 고정해야 한다.
- 영향: Mock과 Production 구현체는 동일 계약 테스트를 사용한다.
