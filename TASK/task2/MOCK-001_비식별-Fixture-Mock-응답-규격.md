...
- Reads: DATA-001~003, API-001~005, SPEC-001·002의 승인 schema
- Writes: versioned 합성 Fixture, manifest와 Mock Adapter 응답
- Side Effects: 개발·테스트 환경의 Mock 응답 제공. Production 외부 호출과 DB 쓰기는 `None`
- Transaction Boundary: None. Fixture 생성·검증 TASK다.
- Idempotency: 동일 Fixture ID·seed·clock·Rule version은 동일 payload와 결과 hash를 만든다.
- Retry Policy: 오류 Fixture는 실제 sleep 대신 fake timer와 결정적 실패 횟수를 사용한다.
...
- Test Gate: TEST-001~005의 schema·상태 Fixture validation과 Production 격리 검사가 통과해야 한다.
- NFR Gate: NFR-004·006의 비식별·비밀정보·비용 기준을 충족해야 한다.
- Evidence Location: Fixture manifest, schema checksum, 결정성 hash와 Production bundle 검사 결과
...
- Depends on: DATA-001·002, API-001·002·003·005, SPEC-001·002의 M1 schema
- Depends on(M2 Extension): DATA-003, API-004, SPEC-001·002의 M2 schema
- Blocks: COMMAND-002·003, QUERY-001·002·004, TEST-001~003·005·006, UI-002~007
...
### 2026-08-26 — SPEC 계약 기반 Mock 완결

- 결정: MOCK-001은 DATA·API·SPEC 계약만 의존하고 Query·Command Logic에는 의존하지 않는다.
- 근거: Step 1에서 Mock을 완결해 Backend 구현 전 Frontend와 계약 테스트가 병렬로 시작할 수 있어야 한다.
- 영향: ViewModel과 이벤트 Fixture는 SPEC-001·002의 schema checksum을 사용한다.
