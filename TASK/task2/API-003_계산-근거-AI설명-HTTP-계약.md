...
- Reads: DATA-001·002, API-001과 승인된 계산·Evidence 정책
- Writes: 계산·근거·AI 설명 HTTP 요청·응답·오류 schema
- Side Effects: 계약 작성은 `None`. 계산 저장과 AI 호출은 후행 Command가 담당한다.
- Transaction Boundary: 계산 생성과 조회·설명 호출의 경계를 서로 분리한다.
- Idempotency: 계산 생성의 idempotency key와 조회의 안전한 반복 의미를 정의한다.
- Retry Policy: 조회와 AI 일시 장애만 제한적으로 재시도하고 계산 validation 오류는 재시도하지 않는다.
...
- Test Gate: TEST-002·003·005의 계산·근거·인가 계약 검사가 통과해야 한다.
- NFR Gate: NFR-001·002·004·006의 성능·결정성·보안·비용 기준을 충족해야 한다.
- Evidence Location: versioned HTTP schema, 오류 코드표, 계약 테스트와 응답시간 증거
...
- Depends on: DATA-001·002, API-001
- Blocks: MOCK-001, COMMAND-003·009, QUERY-002, UI-004~006, TEST-002·003
- External Blockers: Net Benefit 정책 9개와 동률 처리 규칙 승인
...
- 결정: API-003에 Execution Contract와 Verification Gates를 추가한다.
- 근거: 계산·조회·AI 호출의 부작용과 재시도 경계를 구현 전에 분리해야 한다.
- 영향: AI 설명 장애는 결정론적 계산·근거 계약을 변경하지 않는다.
