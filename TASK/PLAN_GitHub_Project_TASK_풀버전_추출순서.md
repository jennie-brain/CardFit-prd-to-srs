...
| **Step 5** | UI/UX (`UI`) | 확정된 계약과 상태 모델을 사용해 정상·로딩·빈 상태·부분 데이터·오류 화면을 설계한다. | 입력, 계산 결과, 유지·변경 결론, 근거, 이행 계측 UI TASK | 핵심 사용자 여정과 접근성, 모든 서버 상태의 화면 대응이 확인되어야 한다. |
...
flowchart LR
    ...
    S1 --> S5["Step 5 UI/UX<br/>상태·응답 계약"]
    S2 --> S5
    S3 --> S5
    S4 --> S5
```
...
권장 순서는 `계약·데이터 → 로직 → 테스트 → 비기능 → UI/UX`이다. 이 순서를 따르면 프론트엔드는 Mock 계약을 기반으로 병렬 작업할 수 있고, 후행 구현이 데이터 구조나 오류 상태를 다시 추측하는 일을 줄일 수 있다. 다음 작업은 Step 1의 TASK 본문을 즉시 생성하는 것이 아니라, 먼저 Step 1에서 추출할 DATA/API/MOCK 후보 목록과 중복·의존성을 사용자에게 제시하고 승인을 받는 것이다.
