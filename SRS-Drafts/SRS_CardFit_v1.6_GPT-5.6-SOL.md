# CardFit 소프트웨어 요구사항 명세서 (SRS) — 작성 AI: GPT-5.6 SOL

| 항목 | 내용 |
| --- | --- |
| 문서 ID | SRS-CARDFIT-MVP-001 |
| 개정 버전 | 1.6 |
| 작성 AI 모델 | GPT-5.6 SOL (Codex) |
| 제품 런타임 기본 AI | Google Gemini 2.5 Flash-Lite (`AI_MODEL`, M2 선택 기능) |
| 날짜 | 2026-08-25 |
| 기준 서식 | 예시 SRS 문서(AD-Core-Platform SRS)의 7섹션 포맷 |
| 참고 표준 | ISO/IEC/IEEE 29148:2018 (예시 포맷을 벗어나는 내용에 한해 8·9·11장에서 인용) |
| 원천 문서 | `SRS_CardFit_v1.5.md`(직전 기준선), `PRD/PRD_CardFit_v1.3.md`(제품 기준선), `MVP-개발목표-적절성-종합-검토(난이도_가능성_효율성)-보고서.md`, `team-project_2nd/master-deck`, C-TEC-001~007 |

> 본 버전의 기준 문서는 Markdown 원본 `SRS-Drafts/SRS_CardFit_v1.6_GPT-5.6-SOL.md`다.

---

## 개요 (Summary)

CardFit은 뱅크샐러드의 공개된 서비스 경험을 바탕으로 제안한 **비공식 내부 기능 확장 역기획**이다. 구현물은 비식별 Fixture와 Mock Platform Adapter로 전체 흐름을 실행하는 독립 검증 MVP이며, 실제 도입에서는 Adapter 구현체만 내부 인터페이스로 교체한다. 본 SRS는 `PRD/PRD_CardFit_v1.3.md`를 제품 기준선으로 사용하며 실제 내부 시스템·API·조직·기술 스택을 알고 있거나 대변하지 않는다.

**v1.6의 종합 판정은 조건부 구현 가능이다.** 지정 스택은 유지하되 모든 요구사항을 한 번에 구현하지 않는다. `M0 핵심 가치 프로토타입 → M1 저장 가능한 포트폴리오 MVP → M2 자동화 베타 → M3 실제 통합`의 승격 구조를 적용한다. 사용자가 미래지출을 입력하고 현재 카드 유지 또는 변경 결론과 계산 근거를 이해하는 경험은 M0부터 보존한다. Prisma·Supabase 영속화는 M1, Gemini 설명과 Vercel Cron 기반 이행 관측은 M2에서 활성화한다.

- **기능 요구사항**: REQ-FUNC 13건(Must 7·Should 4·Could 2) — PRD 3장의 Given/When/Then 인수기준을 조건-결과 요약형으로 정리했으며, 문항별 출처는 [PRD]/[Derived]/[Design Decision]/[TBD]로 구분한다(4.1 서두 참조)
- **비기능 요구사항**: REQ-NF 9건 — 성능(p95 5초)·신뢰성(오류율 0.1%, M2 가용성 99.5%)·보안(오조회 0건)·환경별 비용 상한
- **제품 측정 요구사항**: REQ-METRIC 11건·REQ-GR 6건 — 자기보고·마이데이터 관측, 행동 완주·유지 준수·조합안 이행을 분리하고 판정 불가를 실패로 집계하지 않는다
- **구현 기술 요구사항**: C-TEC 7건 — Next.js 단일 풀스택, Prisma·Supabase, Tailwind·shadcn/ui, Vercel AI SDK·Gemini, Vercel Git 배포
- **속도·비용 통제**: 포트폴리오 MVP는 월 $0~2 목표, 업무·상업 베타는 월 $50 기본 상한으로 운영하며 초과 예상 시 기능 승격을 중단한다
- **초기 합격 범위**: M1까지를 포트폴리오 MVP 합격 범위로 정의하고, 업계 평균 초기값·Gemini·Cron·실제 Platform Adapter는 합격 필수조건에서 제외한다
- **기능 커버리지**: Mock Adapter 기준 In-Scope 12개 기능 전체 실행 가능. 실제 통합은 F-02·F-11의 Adapter와 F-13의 카드 상태 의미 확인 필요
- **미해결 통합 조건**: 내부 Identity·MyData·Card Catalog 계약, 명시적 카드 해지 상태, Production 네트워크 방식 — 8장 참조
...
본 문서는 뱅크샐러드 내부 기능 확장을 제안하는 CardFit 독립 MVP의 소프트웨어 요구사항을 정의한다. 독립 MVP는 지정 기술 스택과 Mock Adapter로 실행 가능해야 하며 실제 통합에서는 뱅크샐러드 플랫폼 인터페이스를 재사용한다.
...
- 뱅크샐러드 Platform Adapter 계약과 비식별 Fixture
...
- 뱅크샐러드 기존 인증·마이데이터·상품 정본의 재구현 및 운영 인프라 변경

#### 1.2.1 단계별 구현 기준선

| 단계 | 목적 | 필수 기능·기술 | 명시적 제외 | 완료 조건 |
| --- | --- | --- | --- | --- |
| **M0 핵심 가치 프로토타입** | 미래지출에 따라 유지·변경 판단이 달라지는지 가장 빠르게 검증 | Next.js·TypeScript·Tailwind·shadcn/ui, 3개 사용자 Fixture, 카드 10~20개 Seed, 혜택 규칙 2~3종, 최대 3장 조합, F-01·03·04·05·06·12 | 인증, DB 영속화, Gemini, Cron, 실제 연동 | 정상·경계·실패 Fixture E2E 통과, 사용자에게 결론·차액·근거 6개 이상 표시 |
| **M1 저장 가능한 포트폴리오 MVP** | 재현 가능한 시연과 데이터 모델 검증 | M0 + Prisma ORM·로컬 Supabase CLI·Supabase Free, Mock Adapter, 선택·계산 스냅샷 저장, F-02·08·09 | 업계 평균 초기값, AI 필수화, 자동 이행 관측, 실제 연동 | 로컬 migration·seed 성공, Vercel Preview 배포, 핵심 UX 검증 8개 통과 |
| **M2 자동화 베타** | 제한된 사용자에게 운영·측정 가능성 검증 | Gemini 선택 설명, 일 1회 Vercel Cron, F-11의 개인 이력 분기, F-13 자기보고·관측 Fixture, 비용·Guardrail 대시보드 | 외부 알림, 대규모 크롤링, 실제 내부 API | 컴플라이언스 조건 확인, 멱등·상태전이·비용 상한 테스트 통과 |
| **M3 실제 통합** | 뱅크샐러드 환경에서 도입 가능성 검증 | Production Adapter, 내부 Identity·Consent·MyData·Catalog 계약 | 확인되지 않은 소비자 API 직접 호출 | 계약·보안·네트워크·카드 상태 의미 승인 후 별도 통합 합격 판정 |

M0는 설계 위험을 낮추기 위한 개발 단계이며 배포 가능한 CardFit MVP의 최종 합격선은 M1이다. 따라서 M0에서 DB를 생략하는 것은 C-TEC-003 위반이 아니지만, M1 완료를 주장하려면 Prisma와 로컬·배포 Supabase 구성이 반드시 검증되어야 한다. M2·M3 기능의 미구현은 M1 기능 커버리지 실패로 계산하지 않는다.
...
| Platform Adapter | Identity·Consent·HeldCard·PastSpend·Card Catalog를 동일 도메인 계약으로 제공하는 경계. MVP는 Mock, 실제 통합은 뱅크샐러드 내부 구현체를 사용한다 |
| Fixture | 개인정보 없이 정상·실패·부분·오래된 데이터 상태를 재현하는 비식별 테스트 데이터 |
| 해지 추정 | 명시적 해지 상태가 없을 때 연결·동기화·완전성이 정상인 두 차례 관측에서 대상 카드가 연속 미관측된 상태. 확정 검증과 구분한다 |
...
| MVP 개발 엔지니어 | CardFit 개발팀 | Next.js 단일 앱, 계산 엔진·Adapter·조합 최적화·이행 계측 구현 |
...
| 뱅크샐러드 플랫폼 팀 | 내부 플랫폼 | Identity·MyData·Card Catalog·Analytics Adapter 계약 및 실제 통합 구현 제공 |
...
| 외부 시스템 | 연동 내용 | 제약 |
| --- | --- | --- |
| Identity·Consent Adapter | 사용자 컨텍스트, 동의 상태·범위·기준일 | MVP는 Mock 세션, 실제 통합은 내부 계약 필요 |
| HeldCard·PastSpend Adapter | 보유카드·과거 소비와 `data_as_of`·`sync_status`·`completeness` | 실패·부분 응답을 빈 배열로 변환 금지 |
| Card Catalog Adapter | 상품·혜택·규칙 버전 | MVP는 Seed, 실제 통합은 내부 상품 정본 계약 필요 |
| Analytics Adapter | 제품 이벤트 수집 확인 | MVP는 DB 이벤트 로그, 실제 통합은 내부 분석 플랫폼 |
...
| 구성요소 | 역할 |
| --- | --- |
| Platform Adapter 계층(F-02) | Mock/Production 구현체로 Identity·Consent·HeldCard·PastSpend·Catalog를 동일 계약으로 제공 |
| Fixture 모듈 | 정상·부분·오래된 데이터·동의 만료·연결 해제·해지 추정 시나리오 제공 |
...
| `PlatformAdapter` | `getIdentity`, `getConsent`, `fetchHeldCards`, `fetchPastSpend`, `getCardCatalog` | 모든 응답에 `data_as_of`·`sync_status`·`completeness`·`source` 포함 |

### 3.5 Use Case Diagram

아래 다이어그램은 3.2~3.4의 액터(사용자·Platform Adapter·카드사 시스템·데이터 운영·계산 품질·컴플라이언스)와 기능 요구사항 간 관계를 나타낸다.

```mermaid
flowchart LR
    User([사용자])
    MyData([Platform Adapter])
    Card([카드사 시스템])
    DataOps([데이터 운영])
    Quality([계산 품질])
...
sequenceDiagram
    ...
    participant 마이데이터API as Platform Adapter
...
sequenceDiagram
    ...
    participant API as 시스템(API)
    participant MyData as HeldCard Adapter
...
sequenceDiagram
    ...
    participant 클라이언트
    participant 마이데이터모듈 as 마이데이터 연동 모듈
    participant 마이데이터API as Consent·MyData Adapter
...
sequenceDiagram
    ...
    alt 과거 이력 3개월 이상 보유
        ...
        마이데이터모듈-->>초기값모듈: 이력 부족
        초기값모듈-->>클라이언트: 빈 입력 폼 + 비개인화 예시(REQ-FUNC-008 AC3)
    end
    사용자->>클라이언트: 제안값 확인·수정
```
...
flowchart TB
    ...
    subgraph External["외부 시스템"]
        MyDataAPI["Platform Adapter<br/>Mock 또는 실제 구현체"]
        CardCo["카드사 시스템(8개사)"]
    end
...
flowchart TD
    ...
    C -- "없음" --> E["빈 입력 폼·비개인화 예시<br/>(REQ-FUNC-008 AC③)"]
...
| **REQ-FUNC-008** | 과거 패턴 기반 초기값 자동 제안 | US-D / F-11 | Should (M2) | Functional | 1) A/B 테스트(n=500, 250/250) 2) QA 검증 | ① 과거 이력 3개월 이상이면 개인 이력 기반 제안과 기준 기간을 표시한다 ② 온보딩 완료율이 대조군 대비 +15%p 이상 개선되어야 Must 승격을 검토한다 ③(실패) 이력 3개월 미만·부재 시 업계 평균을 임의 생성하지 않고 빈 입력 폼과 비개인화 예시만 제공한다 ④ 승인된 출처·최신성·편향 검토가 확보된 경우에만 업계 평균 분기를 별도 변경요청으로 활성화한다 | Confirmed — M1 제외, M2 실험 | 개발 엔지니어 |
...
| **REQ-FUNC-010** | 추천안 이행 계측(측정 전용) | US-C / F-13 | Should (M2) | Functional | 1) 시간 시뮬레이터·상태전이 테스트 2) Cron·식별·연속관측 테스트 3) 멱등성 테스트 4) QA 검증 | ① M1은 시간 시뮬레이터로 선택 시 기준 스냅샷과 +30일 상태전이를 재현한다 ② M2에서 일 1회 Cron이 검증 대상을 만들고 승인된 범위 안에서 Adapter 관측을 수행한다 ③ 자기보고와 관측 상태를 독립 저장한다 ④ 명시적 발급·해지 상태는 정상 관측 1회로 검증할 수 있다 ⑤ 명시적 해지 상태가 없고 연결·동기화·완전성이 정상인 24시간 이상 간격의 두 관측에서 연속 미관측되면 `INFERRED_CANCELLED`로 기록하며 검증 완료로 승격하지 않는다 ⑥ 유지 추천은 `VERIFIED_MAINTAINED`로 분리하고 행동 완주율에서 제외한다 ⑦ 부분응답·오래된 데이터·약한 식별·충돌은 `INCONCLUSIVE` 또는 `UNAVAILABLE`로 기록한다 ⑧ 동일 `selection_id` 중복 제출은 최초 1건만 유효하다 | Confirmed — M1 시뮬레이션, M2 자동화 | 개발 엔지니어 |
...
| **REQ-NF-004** | 월 가용성 | PRD §5 신뢰성 | Should (M2) | Reliability | Vercel·애플리케이션 상태 점검 | M1은 배포 시 smoke test와 수동 상태 확인을 통과한다. 월 가용성 ≥ 99.5%와 1분 주기 헬스체크는 업무 베타 M2부터 적용한다. 다운타임은 5xx 또는 10초 초과 타임아웃 연속 3회로 정의한다 | Confirmed — 단계 분리 | 시스템 운영자 |
...
| **REQ-NF-007** | 비용 관리(외부 호출) | PRD §5 비용 | Must | Cost | 공급자 사용량·자체 원장 월간 대조 | M1 포트폴리오는 월 $2, M2 업무 베타는 월 $50을 기본 상한으로 한다. 예상 비용이 상한의 80%에 도달하면 Gemini 신규 생성과 비필수 Preview DB 생성을 중단하고, 100% 도달 시 외부 유료 호출을 차단한다. 실제 MyData 내부 비용은 계약 전 별도 미산정으로 표시한다 | Confirmed — 환경별 상한 | 제품(PM) |
| **REQ-NF-008** | 최소 감사 로그 | PRD §5 보안 | Must | Security | 로그 필드·마스킹 점검 | M1은 계산 ID·결과 해시·`rule_version`·시간·오류 코드만 저장하고 원본 금융 응답은 저장하지 않는다. M2 이상에서 입력 스냅샷과 Adapter 응답 코드를 추가한다. 보존 기간·접근 권한·삭제 정책이 승인되기 전에는 실제 개인신용정보를 수집하지 않는다 | Confirmed — 최소 수집·단계 확장 | 컴플라이언스·보안 |
| **REQ-NF-009** | Guardrail 모니터링 | PRD §5 모니터링 | Must | Observability | 대시보드·수동 점검 테스트 | M1은 관리자 대시보드에서 GR1~GR5와 오조회 상태를 확인하고 배포 전·시연 전 수동 점검한다. 5분~24시간 능동 알림 SLA와 외부 Slack·이메일 알림은 M2 운영 승인 후 적용한다 | Confirmed — M1 수동, M2 자동 | 계산 품질/데이터 운영/PM/컴플라이언스 |
...
classDiagram
    ...
    class PlatformDataAdapter {
        <<interface>>
        +getIdentity(context) Identity
        +getConsent(userId) ConsentStatus
        +fetchHeldCards(userId) AdapterResult
        +fetchPastSpend(userId) AdapterResult
    }
    class MockPlatformAdapter {
        +loadFixture(caseId) AdapterResult
    }
    class BanksaladPlatformAdapter {
        +mapInternalResponse() AdapterResult
    }
    ...
    CalculationEngine --> PlatformDataAdapter : 데이터 조회
    MockPlatformAdapter ..|> PlatformDataAdapter
    BanksaladPlatformAdapter ..|> PlatformDataAdapter
    ...
    DefaultSuggestionEngine --> PlatformDataAdapter : 과거이력 조회
...
| US-A / F-02 | REQ-FUNC-002 | PlatformDataAdapter·MockPlatformAdapter | TC-FUNC-002 | 미실시 |
...
| PRD §5 비용 | REQ-NF-007 | CostDashboard | TC-NF-007 | 미실시(M1 $2·M2 $50 상한) |
| PRD §5 보안 | REQ-NF-008 | AuditLogStore | TC-NF-008 | 미실시(M1 최소 로그·M2 확장) |
| PRD §5 모니터링 | REQ-NF-009 | GuardrailDashboard·AlertDispatcher | TC-NF-009 | 미실시(M1 수동·M2 자동) |
...
- 실제 통합의 `HeldCard`·`PastSpend`는 승인된 Platform Adapter가 제공하는 개인신용정보다. 독립 MVP는 비식별 Fixture만 사용하며 CardFit은 동의 범위 안의 파생값과 최소 스냅샷만 저장한다.
...
- M1의 `Calculation`은 계산 ID·결과 해시·`rule_version`·시간·오류 코드만 보존한다. 입력 스냅샷과 Adapter 응답 코드는 승인된 보존·삭제 정책이 적용되는 M2부터 추가한다(REQ-NF-008).
...
| ② Platform Adapter 일시 장애 | 동의는 `유효`이나 Adapter가 타임아웃·오류를 반환하고 직전 확인된 캐시가 존재 | 캐시된 최신 데이터로 계산을 계속하고 "최근 확인된 데이터 기준" 경고 + 기준일 표시 | REQ-FUNC-004 AC3 |
...
| :---: | --- | --- | --- | :---: |
| A5 | 실제 도입 단계에서 내부 Identity·MyData·Card Catalog Adapter 계약과 접근 승인을 확인할 수 있다 | 0순위 — 실제 통합 전 | 실제 데이터 통합. Mock MVP에는 영향 없음 | 계약 테스트·컴플라이언스 검토 |
...
| ADR-004 | 인증·마이데이터·상품 정본을 재구현하거나 소비자 앱 내부 API를 직접 호출하지 않고 Platform Adapter를 사용한다 | 내부 확장 제안의 경계를 지키고 확인되지 않은 내부 구조를 추측하지 않는다 | 확정 |
...
| ADR-008 | 제품은 비공식 내부 확장 제안, 구현물은 독립 검증 MVP, 실제 도입은 Adapter 교체라는 3층 구조를 사용한다 | 포트폴리오의 실행 가능성과 기술적 정직성을 동시에 확보한다 | 확정 |

### 8.3 리스크 및 제약사항

| 축 | 리스크/제약 | 대응 | 상태 |
| --- | --- | --- | :---: |
| 실제 통합 | 내부 Adapter 계약·접근 권한·데이터 의미가 확인되지 않음 | 독립 MVP는 Mock으로 검증하고 실제 통합 준비도를 별도 `CONDITIONAL`로 보고 | 통합 전 확인 |
...
- 독립 MVP의 비식별 Fixture·Mock Platform Adapter 계약
- 실제 도입의 Identity·Consent·HeldCard·PastSpend·Card Catalog·Analytics Adapter 계약과 접근 승인
...
본 절은 SRS를 구현 가능한 시스템 기준선으로 구체화한다. **C-TEC-001~007은 CardFit 독립 MVP에 반드시 적용한다.** 뱅크샐러드 프로덕션 전체의 스택이나 배포 플랫폼을 규정하지 않으며, 실제 통합은 Production Adapter와 네트워크 제약으로 별도 관리한다.

| ID | 제약조건 | 구현 기준 |
| --- | --- | --- |
| **C-TEC-001** | Next.js App Router 기반 단일 풀스택 | UI·서버 렌더링·서버 명령·HTTP API를 하나의 Next.js 프로젝트에 배치하고 별도 CardFit 백엔드를 두지 않는다 |
| **C-TEC-002** | Server Actions 또는 Route Handlers 사용 | DB 접근·계산·Adapter 호출은 서버 경계에서만 수행한다 |
| **C-TEC-003** | Prisma ORM + Supabase CLI·PostgreSQL | M0는 정적 Fixture를 허용한다. M1부터 로컬은 Supabase CLI PostgreSQL, 배포 환경은 Supabase PostgreSQL과 하나의 Prisma schema·migration을 사용한다. Preview 전용 DB는 M2 또는 병렬 검증 필요가 확인될 때 생성한다 |
| **C-TEC-004** | Tailwind CSS + shadcn/ui | design token과 공용 UI 컴포넌트를 사용하며 임의 스타일을 제한한다 |
| **C-TEC-005** | Vercel AI SDK 기반 LLM 오케스트레이션 | 별도 Python 서버 없이 Next.js 서버 전용 AI Adapter에서 수행한다 |
| **C-TEC-006** | Gemini 기본, 환경 변수로 모델 교체 | `AI_PROVIDER`, `AI_MODEL`, API key를 코드와 분리한다 |
| **C-TEC-007** | 독립 MVP의 Vercel 단일 배포 | CI/CD 서버 없이 Git Push로 Preview·Production 배포를 자동화한다 |
...
| 외부 연결 | Mock/Production Platform Adapter, Gemini API, 카드사 공식 링크 | 소비자 앱 내부 API 무단 호출, 요구사항에 없는 외부 SaaS·메시지 브로커·알림 사업자 |
...
| 뱅크샐러드 플랫폼 연동 | 조건부 적합 | MVP는 Mock Adapter, 실제 통합은 동일 계약의 내부 Adapter | 소비자용 API 직접 호출 또는 별도 중계 Backend 신설은 금지 |
...
현재 스택 밖 가능성은 외부 채널 능동 알림, 장시간 Headless Browser 수집, 전용망·mTLS·고정 IP가 필요한 Production Adapter다. 앞의 두 항목은 MVP 범위에서 제외한다. 마지막 항목은 CardFit 기능 결함이 아니라 실제 통합 제약으로 기록하고 뱅크샐러드 플랫폼 팀의 배포 방식에 맞춘다. F-13은 Vercel Cron으로 검증 대상을 만들고, 사용자 재방문 시 in-app 자기보고를 한 번 수집한다.

#### 8.7.3 Production Integration Constraints

- Mock과 Production Adapter는 동일한 도메인 DTO와 오류 상태를 사용한다.
- Production Adapter의 인증·네트워크·호출 예산은 내부 계약 확정 전 특정하지 않는다.
- 실제 사용자 ID, 인증서·토큰, 원본 금융 데이터는 CardFit DB·브라우저·AI 요청에 저장하거나 노출하지 않는다.
- 명시적 카드 해지 상태가 있으면 검증 완료로 판정한다. 상태가 없지만 연결·동기화·완전성이 정상이고 24시간 이상 간격의 두 관측에서 연속 미관측되면 `INFERRED_CANCELLED`, 그 외에는 `INCONCLUSIVE` 또는 `UNAVAILABLE`로 판정한다.
...
flowchart LR
    ...
    Routes --> Adapter[Platform Adapter]
    Adapter --> Fixture[비식별 Fixture]
    Adapter -. 실제 통합 .-> Banksalad[뱅크샐러드 내부 플랫폼]
...
- Prisma Client, DB URL, Gemini key와 Platform Adapter credential은 서버 전용 모듈에서만 사용한다.
- Supabase에는 CardFit 파생 데이터와 최소 스냅샷만 저장하고 원본 금융 데이터·인증서·토큰을 저장하지 않는다.
...
| **REQ-DATA-001~007** | Prisma schema·migration 기준선, Local Supabase, pooled/direct 연결 분리, 환경별 데이터 격리 | M1의 `supabase start`·`prisma migrate dev/deploy` 성공, Preview 빌드의 Production 쓰기 0건, M2부터 Preview·Production DB 분리 |
...
| REQ-DATA-007 | M1은 Local과 Production을 분리하고 Preview에서 Production 쓰기를 금지한다. M2부터 Preview·Production DB를 물리적으로 분리한다 |
...
| REQ-DEPLOY-004 | M1 Preview는 Fixture·읽기 전용 설정을 사용하고, M2부터 Preview·Production 환경 변수와 DB를 분리한다 |
...
M1의 Preview는 비식별 Fixture와 읽기 전용 설정을 사용하며 Production DB 쓰기를 금지한다. M2부터 Preview와 Production에 서로 다른 `DATABASE_URL`, `DIRECT_URL`, Gemini key와 Platform Adapter 설정을 사용한다. Migration은 Expand → Data Migration → Contract 순서의 하위 호환 방식으로 작성한다. 다음 조건이 모두 충족되어야 기술 제약 구현이 완료된 것으로 판정한다.
...
### 8.13 개발 효율성과 운영 비용 제약

#### 8.13.1 개발 시간 예산

| 단계 | 초급 개발자·AI 보조 기준 목표 | 중단·축소 조건 |
| --- | --- | --- |
| M0 | 3~7일 | 7일 안에 핵심 E2E가 동작하지 않으면 카드·규칙 Fixture 수를 줄이고 DB·AI 작업을 시작하지 않는다 |
| M1 | M0 이후 2~3주 | 3주를 넘기면 F-08·F-09를 후순위화하고 계산·근거·저장 경로를 우선 완료한다 |
| M2 | M1 이후 3~5주 | 정책·동의·관측 상태 의미가 승인되지 않으면 Cron과 실제 관측을 활성화하지 않는다 |
| M3 | 사전 산정 불가 | 내부 계약과 네트워크 조건이 확인되기 전 일정·완료율을 약속하지 않는다 |

AI 코딩은 화면 골격·schema 초안·테스트 생성에 사용하되, Net Benefit 계산식·금융 데이터 권한·관측 판정 규칙은 사람이 승인한다. 한 번에 하나의 수직 경로를 완성하며 `입력 → 계산 → 유지/변경 결론 → 차액 → 근거`가 깨진 상태에서 부가기능을 추가하지 않는다.

#### 8.13.2 외부 서비스 사용 원칙

| 서비스 | M1 | M2 이상 | 비용·복잡성 통제 |
| --- | --- | --- | --- |
| Vercel | 개인·비상업 포트폴리오는 Hobby | 업무·상업 베타는 Pro | 고정 IP·전용망 요구가 생기면 M3 통합 제약으로 이관 |
| Supabase | Free 프로젝트 1개 + Local CLI | Pro 프로젝트 1개, 필요 시 Preview 추가 | Realtime·Storage·Edge Functions는 요구가 생기기 전 사용하지 않음 |
| Prisma | ORM·migration만 사용 | 동일 | Accelerate·Prisma Postgres 등 별도 유료 서비스를 기본 범위에서 제외 |
| Gemini | 기본 비활성, 정형 설명 사용 | Flash-Lite 계열 모델을 AI SDK로 선택 사용 | Evidence DTO당 1회 생성·캐시, 월 $5 AI 하위 상한, 실패 시 정형 문구 |
| Platform Adapter | Mock·Fixture | Production은 M3에서 승인 후 교체 | 인증·호출단가·데이터 의미 미확정 비용을 제품 클라우드 비용과 분리 |

Next.js·shadcn/ui·Prisma ORM·Supabase의 핵심 구성요소는 오픈소스 자산을 활용할 수 있지만 Vercel 호스팅과 Gemini API는 관리형 상용 서비스다. 초급자 MVP에서는 비용 절감을 목적으로 Supabase 자체 호스팅이나 별도 VM을 도입하지 않는다. 이는 운영 복잡성을 높이고 C-TEC-007의 단일 배포 원칙을 훼손한다.

#### 8.13.3 월 비용 예산과 산식

| 운영 시나리오 | 기본 구성 | 월 목표 비용 | 승격 조건 |
| --- | --- | ---: | --- |
| M1 포트폴리오 | Vercel Hobby + Supabase Free + Gemini 비활성/무료 | **$0~2** | 개인·비상업 사용, 비식별 Fixture |
| 소규모 비상업 검증 | M1 + 제한된 Gemini 호출 | **$0~3** | 공급자 무료 한도와 quota 확인 |
| M2 업무·상업 베타 | Vercel Pro + Supabase Pro + 제한된 Gemini | **약 $45~50** | 비용 대시보드·상한·중단 규칙 활성화 |
| M2 Preview DB 상시 분리 | Pro 구성 + 두 번째 Supabase Micro 상당 | **약 $55 + AI** | 병렬 QA가 실제로 필요한 경우에만 승인 |

가격은 2026-08-25 공식 공개 가격을 기준으로 한 계획값이며 세금·환율·실제 MyData 내부 비용은 제외한다. Gemini 설명 1회당 입력 2,000·출력 700 token을 가정하면 Flash-Lite 공개 단가 기준 약 $0.00048이며, 5,000회는 약 $2.40이다. 호출량·모델·가격이 바뀌면 `CostLedger`의 실측값으로 월 예측을 다시 계산한다.

비용 중단 규칙은 다음과 같다.

1. 월 상한 80% 도달 시 Gemini 신규 설명 생성을 끄고 정형 설명으로 전환한다.
2. Preview DB는 기본적으로 상시 운영하지 않으며 M2 병렬 QA 기간에만 활성화한다.
3. Cron은 M2에서 하루 1회만 실행하고 사용자별 타이머나 상시 Worker를 두지 않는다.
4. 외부 서비스 추가는 월 비용·개인정보·장애 시 fallback을 ADR에 기록한 뒤 승인한다.
5. 실제 Platform Adapter 비용은 계약 확인 전 `UNKNOWN`으로 유지하며 $45~50 베타 비용에 포함됐다고 표현하지 않는다.
...
- Mock Adapter의 필수 Fixture 시나리오 미통과 = 독립 MVP 베타 진입 불가
- 실제 통합은 Identity·MyData·Card Catalog 계약과 후속 관측 목적의 컴플라이언스 승인 전 활성화 불가

### 9.4 MVP 기능 커버리지

| 기능 | 최초 적용 단계 | 독립 구현 | 실제 통합 의존성 | 단계별 판정 |
| --- | :---: | --- | --- | :---: |
| F-01 미래지출 입력 | M0 | Next.js Form + Server Action | 없음 | M1 필수 |
| F-02 플랫폼 데이터 연동 | M1 | Mock Platform Adapter | Identity·MyData 계약 | M1 Mock 충족·M3 조건부 |
| F-03 세 가지 예상 시나리오 | M0 | TypeScript Scenario Engine | 없음 | M1 필수 |
| F-04 조합 최적화·게이팅 | M0 | 결정론적 TypeScript Engine | Card Catalog 계약·Net Benefit 정책 | M1 최소 규칙 필수·M3 조건부 |
| F-05 결제수단 배분 | M0 | Allocation Engine | 없음 | M1 필수 |
| F-06 근거 공개 | M0 | Server Component·Route Handler | Rule 정본 | M1 필수·M3 조건부 |
| F-07 단계적 전환 | M1 | 결과 정렬 | 없음 | M1 선택 |
| F-08 자유 입력 | M1 | UI·정규화 | 없음 | M1 선택 |
| F-09 범위 입력 | M1 | UI·검증 | 없음 | M1 선택 |
| F-11 초기값 제안 | M2 | PastSpend Fixture, 이력 없으면 빈 폼 | PastSpendAdapter·업계 평균 승인 | M1 제외·M2 실험 |
| F-12 스코프 고지 | M0 | UI·금지어 검사 | 없음 | M1 필수 |
| F-13 추천안 이행 | M2 | M1 시간 시뮬레이터, M2 자기보고·관측 Fixture·Cron | 카드 상태·동기화 의미 | M1 합격 제외·M2 조건부 |

**커버리지 결론:** M1은 핵심 가치 전달에 필요한 7개 필수 경로(F-01·02 Mock·03·04 최소 규칙·05·06·12)를 모두 구현할 수 있다. 전체 12개 기능은 M2 Mock 기준으로 실행 가능하지만, 이를 M1 합격 조건으로 두지 않는다. 이 단계 구분은 기능을 폐기하는 것이 아니라 초급 개발자가 가치 검증 전에 운영 자동화에 시간을 소모하지 않게 한다. Mock 성공은 프로덕션 통합 성공으로 간주하지 않는다.

### 9.5 핵심 사용자 경험 보존 검증

CardFit의 핵심 가치 전달 흐름은 `미래지출 1회 입력 → 세 가지 예상 범위 → 카드 조합 또는 유지 결론 → 차액·근거 검증`이다. `추천안 이행 확인`은 가치 전달 이후의 측정 경로이므로 M2로 분리해도 핵심 사용자 경험이 훼손되지 않는다.

| ID | 검증 시나리오 | 합격 조건 |
| --- | --- | --- |
| UX-V-001 | 예상금액 1회 입력 | 추가 입력 없이 LOW·BASE·HIGH 생성 |
| UX-V-002 | 기본 결과 | `예상한 만큼 쓸 때`를 첫 화면에 표시 |
| UX-V-003 | 조건부 추천 | 시나리오별 유지·변경 조건 설명 |
| UX-V-004 | 근거 | 근거 6개·기준일·미반영 비용 제공 |
| UX-V-005 | Adapter 장애 | 빈 카드가 아닌 오류·기준일 표시 |
| UX-V-006 | Gemini 장애 | 계산·근거·선택 정상, 정형 설명 폴백 |
| UX-V-007 | 유지 추천 | 유지 준수로 기록하고 행동 완주율에서 제외 |
| UX-V-008 | 해지 상태 부재 | 추정·판정 불가로 표시하고 확정 문구 금지 |

**검토 결과: PASS.** Adapter는 데이터 출처를 교체할 뿐 계산과 결정 흐름을 제거하지 않는다. AI를 설명 전용으로 제한하고 해지 확정·추정·판정 불가를 구분하므로 핵심 사용자 경험과 신뢰성이 함께 보존된다.

### 9.6 실제 통합 준비도

| 평가 축 | 상태 | 근거 |
| --- | :---: | --- |
| M1 핵심 기능 커버리지 | `SATISFIED` | 핵심 가치 필수 경로 7/7 구현 가능 |
| M2 확장 기능 커버리지 | `CONDITIONAL` | 전체 12개 기능은 Mock 실행 가능하나 정책·자동화 승격 조건 필요 |
| 핵심 사용자 경험 보존 | `PASS` | UX-V-001~008 충족 |
| 실제 통합 준비도 | `CONDITIONAL` | Identity·Consent·HeldCard·PastSpend·Card Catalog·Analytics 계약 확인 필요 |
| 프로덕션 스택 적합성 | `NOT_EVALUATED` | 뱅크샐러드 실제 운영 인프라를 확인하거나 규정하지 않음 |

Mock 성공은 실제 통합 성공으로 집계하지 않는다. Production Adapter 계약 테스트, 카드 상태 의미, 동의 목적, 네트워크 제약이 확인된 이후에만 실제 통합 준비도를 `READY`로 변경할 수 있다.

---

## 10. 결론

CardFit v1.6은 제품 제안·독립 검증 MVP·실제 도입을 분리한 비공식 역기획 SRS이며, 여기에 개발 속도와 운영비를 통제하는 M0~M3 승격 구조를 추가했다. 배포 가능한 M1은 Next.js App Router 단일 풀스택, Prisma, Supabase, Tailwind CSS, shadcn/ui와 Vercel Git 배포를 사용한다. Gemini와 Cron은 M2의 선택·자동화 기능이다. 실제 도입에서는 Mock Adapter를 내부 구현체로 교체하며 뱅크샐러드 운영 인프라를 이 스택으로 변경한다는 의미는 아니다.

M1의 핵심 가치 필수 경로 7개와 UX 검증 시나리오는 구현 가능하므로 가치 전달은 PASS다. 전체 12개 기능을 한 번에 개발하는 접근은 개발 효율성 관점에서 FAIL이며, F-11 업계 평균·F-13 자동 관측·AI 설명·능동 알림은 승격 조건을 충족한 뒤 도입한다. 포트폴리오 비용은 월 $0~2, 업무 베타는 약 $45~50을 계획 기준으로 삼되 실제 Platform Adapter 비용은 별도 `UNKNOWN`으로 관리한다.

가장 먼저 확정해야 할 것은 Net Benefit 최소 규칙이다. 계산 기간·임계값·전환비용·동률 규칙이 정해지지 않으면 UI와 인프라가 완성되어도 추천의 타당성을 검증할 수 없다. 실제 통합의 추가 선결조건은 Platform Adapter 계약, 후속 관측의 컴플라이언스 승인과 카드 상태 의미다. 명시적 해지 상태가 없으면 해지를 확정하지 않고 `INFERRED_CANCELLED` 또는 `INCONCLUSIVE`로 모델링한다.
...
| 1.4 | 2026-08-25 | 뱅크샐러드 내부 확장 경계, Mock/Production Platform Adapter, 지정 MVP 스택, 12개 기능 커버리지와 핵심 UX 보존 검증 반영 |
| 1.5 | 2026-08-25 | 비공식 내부 확장 제안·독립 검증 MVP·실제 도입의 3층 구조, 통합 준비도 분리, 해지 추정 상태 정합화 |
| 1.6 | 2026-08-25 | M0~M3 단계별 기준선, 핵심 가치 7개 필수 경로, 업계 평균·AI·Cron 후순위화, 환경별 가용성·감사·알림 기준과 월 비용 상한 반영 |
...
- 원천 PRD: `PRD/PRD_CardFit_v1.3.md`
- 시스템·비용 효율성 검토: `MVP-개발목표-적절성-종합-검토(난이도_가능성_효율성)-보고서.md`
- 변경 실행 계획: `plans/PLAN_CardFit_v1.5_Three_Layer_Portfolio_Alignment.md`
...
- Vercel 플랜·Hobby 제한·Cron 가격: <https://vercel.com/pricing>, <https://vercel.com/docs/plans/hobby>, <https://vercel.com/docs/cron-jobs/usage-and-pricing>
- Supabase 가격·Compute 비용: <https://supabase.com/pricing>, <https://supabase.com/docs/guides/platform/manage-your-usage/compute>
