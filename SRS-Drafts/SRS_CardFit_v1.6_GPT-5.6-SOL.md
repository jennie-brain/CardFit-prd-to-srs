...
| 원천 문서 | `SRS_CardFit_v1.5.md`(직전 기준선), `PRD_CardFit_v1.3.md`(제품 기준선), `MVP-개발목표-적절성-종합-검토(난이도_가능성_효율성)-보고서.md`, `team-project_2nd/master-deck`, C-TEC-001~007 |
...
CardFit은 뱅크샐러드의 공개된 서비스 경험을 바탕으로 제안한 **비공식 내부 기능 확장 역기획**이다. 구현물은 비식별 Fixture와 Mock Platform Adapter로 전체 흐름을 실행하는 독립 검증 MVP이며, 실제 도입에서는 Adapter 구현체만 내부 인터페이스로 교체한다. 본 SRS는 `PRD_CardFit_v1.3.md`를 제품 기준선으로 사용하며 실제 내부 시스템·API·조직·기술 스택을 알고 있거나 대변하지 않는다.
