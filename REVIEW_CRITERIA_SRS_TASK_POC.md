...
## 7. CardFit 수정 후 TASK 수 산정

현재 전체 개발 TASK 기준선은 53개다. 지금까지의 검토 결과를 원자적 TASK에 반영하면 권장 목표는 **72개**다.

| 변경 | 기존 | 수정 | 순증 |
| --- | ---: | ---: | ---: |
| 현재 기준선 | 53 | 53 | 0 |
| 계산 정책 Decision TASK | 0 | 3 | +3 |
| COMMAND-003 계산·최적화·게이팅·배분·저장·이벤트 분리 | 1 | 6 | +5 |
| COMMAND-006 관측 저장·신뢰도·상태전이·집계 분리 | 1 | 4 | +3 |
| COMMAND-007 Rule 등록·후보 만료·회귀 Gate 분리 | 1 | 3 | +2 |
| API-002 입력·제약·선택 Action 계약 분리 | 1 | 3 | +2 |
| API-004 자기보고·자동 관측 계약 분리 | 1 | 2 | +1 |
| API-005 Rule·Health·Cron 운영 계약 분리 | 1 | 3 | +2 |
| 논리 데이터 계약과 Prisma DB 구현 경계 분리 | 0 | 1 | +1 |
| **합계** | **53** | **72** | **+19** |

`DATA-004`와 `API-007`은 현재 M1 Gate에만 나타나는 미존재 ID이므로 위 산정에서 신규 TASK로 세지 않는다. 먼저 오기인지 실제 누락인지 확인한 뒤 다음처럼 처리한다.

- 오기이면 기존 ID로 교체하며 총합은 72개를 유지한다.
- SRS가 요구하는 별도 책임이라면 TASK를 추가하며 총합은 73개 또는 74개가 된다.

TDD 의존성 변경, 요구사항 ID 명시, GitHub Project 필드 추가, 비용·일정 수정은 기존 TASK의 품질 개선이므로 TASK 수를 증가시키지 않는다. M1과 M2 범위를 실제로 별도 Issue로 더 세분화하면 72개보다 증가할 수 있으므로, 72개를 현재 요구사항에 근거한 권장 기준선으로 사용한다.
