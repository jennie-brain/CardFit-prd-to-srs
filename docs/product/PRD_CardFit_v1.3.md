# [CardFit 미래지출 카드설계 기능 확장 제안] v1.3
- Owner 팀: 진정가
- 최종 업데이트: 2026-08-25
- 변경 이력:
  ...
  - v1.2 → v1.3: 제품은 뱅크샐러드 내부 기능 확장 역기획, 구현물은 독립 검증 MVP, 실제 도입은 Platform Adapter 교체라는 3층 구조를 확정하고 외부 인가·제휴 선결 가정을 제거
  - v1.3 정합성 보정(2026-08-27): 이력 부족 사용자의 미래지출 입력은 빈 값으로 시작하고, 사용자가 선택할 때만 수정 가능한 비개인화 예시를 적용한다. 업계 평균 분기는 출처·최신성·편향 검토 승인 후 M2 변경 요청으로 제한

> 출처: `master-deck/p01~p32` 발표 원고, 정본 `효진/0820/final/01_서비스_역기획서.md` 및 `09_PRD_Requirement_Trace_v0.1.md`
> **포트폴리오 고지**: 본 문서는 뱅크샐러드의 공개된 서비스 경험을 바탕으로 작성한 비공식 역기획이며, 실제 내부 시스템·API·조직·기술 스택을 알고 있거나 대변하지 않는다. 구현 산출물은 핵심 가치를 검증하기 위한 독립 실행형 MVP다.
>
> 본 문서에 명시된 기능 범위와 판정 구조는 v1.3의 제품 결정사항이다. D2·D5·D6의 수치는 Owner 승인 전 베타 운영 기본값(Proposed)이다. 아직 운영 이력이 없는 지표는 `미측정`으로 표시하고 8장의 실험을 통해 기준선을 최초 확보한다.

## 문서 요약

CardFit은 뱅크샐러드의 카드 경험에 미래지출 기반 조합 설계를 추가하는 내부 기능 확장 제안이다. 포트폴리오 구현물은 비식별 Fixture와 Mock Platform Adapter를 사용하는 독립 실행형 검증 MVP이며, 실제 도입 시에는 Adapter 구현체만 내부 Identity·MyData·Card Catalog·Analytics 인터페이스로 교체한다. 사용자는 예상금액을 한 번만 입력하며 시스템은 `예상보다 적게 쓸 때(80%)`, `예상한 만큼 쓸 때(100%)`, `예상보다 많이 쓸 때(120%)`를 자동 계산한다.
...
- **AC2**: Given 사용자가 `한 번 쓰는 금액` 또는 `매달 쓰는 금액`과 미래 예상 금액을 입력하고 비교 가능한 과거 기준값이 있는 경우, When 시스템이 값을 처리하면, Then 사용자가 증가·감소 방향을 별도로 선택하지 않아도 기준값과 미래 금액의 차이로 변화 방향·차액을 표시한다(양방향 처리 오류율 0%). 과거 기준값이 없으면 방향을 임의 생성하지 않는다.
...
- **AC4 (실패 케이스)**: Given 마이데이터 과거 소비 이력이 **3개월 미만이거나 존재하지 않는 신규 사용자**에게, When 미래지출 입력 화면에 진입하면, Then 입력값은 비어 있고 비개인화 예시만 안내된다. 사용자가 `예시 금액 적용하기`를 명시적으로 선택한 경우에만 수정 가능한 예시 값이 입력되며, 화면에는 개인 소비 데이터나 업계 통계를 기준으로 한 값이 아님을 표시한다. 승인된 출처·최신성·편향 검토가 확보되기 전에는 업계 평균 분기를 활성화하지 않는다.
- **AC5 (실패 케이스)**: Given 범위 입력 시 `최소 ≤ 예상 ≤ 최대` 조건을 위반한 경우, When 시스템이 값을 검증하면, Then 저장 전 오류 메시지가 표시되고 잘못된 범위가 계산에 반영되는 건수는 **0건**이다.

> US-E(지속 사용 동기, Job E)는 우선순위 5·비MVP로 본 v1.3 범위에서 제외한다(4장 F-10 참고).
...
| :---: | --- | :---: | :---: | --- | --- |
| **F-01** | 미래지출 입력(카테고리·지출 형태·금액·시점) | A·F | **Must** | 경쟁 6곳 중 이 단계를 가진 곳 0곳 — 유일한 차별 진입점 | 없음(최초 입력 화면) |
| **F-02** | 플랫폼 데이터 연동 + 제약조건 입력 | A·D | **Must** | 뱅크샐러드 내부 확장에서는 기존 보유카드·소비 데이터를 재사용한다. 독립 MVP는 동일 계약의 비식별 Fixture로 전체 흐름을 검증한다 | MVP: Mock Adapter·Fixture / 실제 도입: Identity·Consent·HeldCard·PastSpend Adapter 계약 |
...
| **F-11** | 과거 패턴 기반 초기값 자동 제안 | D·A | **Must** | 가장 큰 가정(A1)의 완화 장치. 독립 MVP에서는 비식별 PastSpend Fixture로 검증한다 | F-02 PastSpend Adapter·데이터 완전성 메타데이터 |
...
| F-13 | 추천안 이행 계측(자기보고 + 플랫폼 관측, 측정 전용) | C | Should | 선택률 이후의 행동 완주·유지 준수·판정 불가를 구분한다. 실행 대행은 하지 않는다 | MVP: 관측 Fixture / 실제 도입: F-02 유효 동의, 카드 상태 의미·관측 목적 승인 |
...
- **보안/비용**:
  ...
  - Production Adapter·Gemini 등 외부 호출은 **비용과 호출 한도**가 발생할 수 있으므로 계산 요청 건수를 성과 지표가 아닌 비용 지표로 분리한다. 독립 MVP의 Mock Adapter 호출 비용은 0원으로 집계한다
...
| HeldCard | 카드사, 카드명, 연회비, 발급일, 실적 기준월 | Platform Adapter(MVP Fixture/실제 내부 구현체) |
| PastSpend | 가맹점, 업종코드, 금액, 결제일 | Platform Adapter(MVP Fixture/실제 내부 구현체) |
| FutureSpendPlan | 카테고리, 지출 형태(`ONE_TIME`·`MONTHLY`), 입력 모드(`SINGLE`·`RANGE`), 예상금액, 최소·최대금액, 시점, 확실도 | 사용자(F-01) |
...
| In | Out |
| --- | --- |
| F-01 미래지출 입력 · F-02 플랫폼 데이터 연동 · **F-03 사용자 입력 1회 기반 3개 예상 시나리오 동시 계산** · **F-04 시나리오별 독립 조합 최적화와 탭 제공** · F-05 결제수단 배분 · F-06 근거 공개 · F-07 단계적 전환 제안 · F-08 이벤트 비종속 입력 · F-09 소득·지출 범위 입력 · F-11 초기값 제안 · F-12 스코프 고지 · F-13 자기보고·플랫폼 관측 기반 추천안 이행 계측 | F-10 정기 재진단 알림 · F-14 카드 20장+ 대량 처리 최적화 · 해지·전환 실행 대행·상담·만류 대응 안내 · 신규카드 자동 발급 · 자동결제 · 대출·BNPL · 리텐션 전용 기능 · 외부 이메일·SMS·Push 독려 · 뱅크샐러드 기존 인증·마이데이터·상품 정본 재구현 |
...
| 축 | 리스크 | 대응 | 상태 |
| --- | --- | --- | :---: |
| 통합 | 실제 내부 Adapter 계약·접근 권한·데이터 의미가 확인되지 않음 | 독립 MVP는 Mock Adapter로 검증하고 실제 도입 준비도는 `CONDITIONAL`로 별도 보고 | 실제 도입 게이트 |
...
| 운영 | Production Adapter와 Gemini 호출 비용·제한이 미확정 | 외부 호출량과 비용을 별도 관리하고 성과 지표로 사용하지 않음 | 실제 도입 조건부 |
...
| ADR-004 | CardFit이 인증·마이데이터·상품 정본을 재구현하거나 소비자 앱 내부 API를 직접 호출하지 않고 Platform Adapter를 사용한다 | 내부 확장 제안의 시스템 경계를 지키고 확인되지 않은 내부 구조를 추측하지 않는다 | 확정 |
...
| ADR-009 | 제품은 비공식 내부 기능 확장 제안, 구현물은 독립 검증 MVP, 실제 도입은 Adapter 교체라는 3층 구조를 사용한다 | 포트폴리오의 실행 가능성과 기술적 정직성을 동시에 확보하고 외부 제휴 가정을 최소화한다 | 확정 |

### 제품 전제와 검증 우선순위

| # | 가정 | 검증 우선순위 | 반증되면 무엇이 무너지나 | 검증 |
| :---: | --- | --- | --- | :---: |
| A5 | 실제 도입 단계에서 내부 Identity·MyData·Card Catalog Adapter 계약과 접근 승인을 확인할 수 있다 | 0순위 — 실제 통합 전 | 실제 데이터 통합. 독립 MVP에는 영향 없음 | 계약 테스트·컴플라이언스 검토 |
...
- 독립 MVP: 비식별 Fixture와 Mock Platform Adapter 계약
- 실제 도입: Identity·Consent·HeldCard·PastSpend·Card Catalog·Analytics Adapter 계약과 접근 승인
...
아래 제약은 포트폴리오용 독립 검증 MVP에만 적용하며 뱅크샐러드의 실제 운영 기술을 규정하지 않는다.

| ID | 제약 |
| --- | --- |
| C-TEC-001 | 모든 MVP 기능은 Next.js App Router 기반 단일 풀스택 애플리케이션으로 구현하며 별도 CardFit 백엔드를 두지 않는다 |
| C-TEC-002 | DB 접근·계산·Adapter 호출은 Server Actions 또는 Route Handlers에서 수행한다 |
| C-TEC-003 | Prisma ORM을 사용하고 로컬은 Supabase CLI PostgreSQL, Preview·Production은 분리된 Supabase PostgreSQL을 사용한다 |
| C-TEC-004 | UI와 스타일은 Tailwind CSS와 shadcn/ui를 사용한다 |
| C-TEC-005 | LLM 오케스트레이션은 별도 Python 서버 없이 Vercel AI SDK로 Next.js 내부에서 수행한다 |
| C-TEC-006 | 기본 모델은 Gemini이며 환경 변수와 SDK 표준 인터페이스로 모델을 교체할 수 있어야 한다 |
| C-TEC-007 | 독립 MVP는 Vercel Git 연동으로 Preview·Production 배포를 자동화한다 |

전용망·mTLS·고정 IP·별도 런타임이 필요한 실제 내부 연동은 `Production Integration Constraint`로 분리한다. 이는 MVP 기능 커버리지 실패가 아니라 실제 도입 준비도의 조건부 상태다.

### 계산 정책과 승인 상태

| ID | 결정 항목 | v1.3 정책 | 상태·Owner | 변경 통제 |
| :---: | --- | --- | --- | --- |
...
- Mock Adapter의 정상·오류·부분·오래된 데이터·연결 해제 Fixture 미통과 = 독립 MVP 베타 진입 불가
- 실제 도입은 Platform Adapter 계약과 후속 관측 컴플라이언스 승인 전 활성화 불가
...
- Platform Adapter 계약과 실제 통합 의존성은 `Assumptions and dependencies`에 대응시킨다.
...
CardFit v1.3은 뱅크샐러드 내부 기능 확장 역기획과 독립 실행형 검증 MVP를 분리했다. 사용자가 예상 미래지출을 한 번 입력하면 세 가지 예상 범위를 자동 계산하고, 12개월 Net Benefit 50,000원 미만에서는 현재 조합 유지를 반환한다. 지정 기술 스택은 검증 MVP에만 적용하며 실제 도입은 Platform Adapter 계약과 내부 운영 제약에 따른다.
...
구조·추적성·측정 정의는 보완됐다. 후속 SRS는 본 PRD v1.3을 요구사항 기준선으로 사용하고, 프로토타입 기능 커버리지와 실제 통합 준비도를 별도로 평가해야 한다. D2·D5·D6의 수치는 Owner 승인 전 `[Proposed]`로 전달한다.
...
- 3층 구조 실행 계획: `docs/plans/PLAN_CardFit_v1.5_Three_Layer_Portfolio_Alignment.md`
