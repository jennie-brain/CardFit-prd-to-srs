...
- [`docs/tasks/task2/*.md`](../task2) — 각 TASK의 AC·DoD·Verification Gates
- `docs/technical/SRS_CardFit_v1.6_GPT-5.6-SOL.md` — M1 핵심 가치 경로 정의
...
이 절은 `grill-it` 인터뷰에서 나온 질문을 그대로 기능으로 확정하지 않고, PRD·SRS·TASK의 기존 경계와 대조해 프로토타입 착수 결론으로 좁힌 과정이다. 상세 토픽 상태와 반영 파일은 [`docs/decisions/GRILL_LEDGER.md`](../../decisions/GRILL_LEDGER.md), 화면별 명세는 [`prototype-visual-spec.md`](prototype-visual-spec.md)를 따른다.

### 12.1 계획 범위와 최초 구현 범위의 분리

처음에는 화면 TASK 11건만 별도 시각 슬라이스로 보는 안을 검토했다. 그러나 이 방식은 `prototype-suggestion.md`가 산정한 계약 8건을 계획에서 누락한 것처럼 읽히고, Fixture와 실제 계약이 달라질 때 화면 재작업 위험도 남겼다. 따라서 다음처럼 정리했다.
