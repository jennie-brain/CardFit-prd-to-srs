...
다음 단계                              ← sr-only h2. 화면에는 보이지 않고 screen reader만 읽는다
[카드 조건 확인하기]
```
...
| 행동 | 결과 |
| --- | --- |
| `결과 확인하기` | **연회비 한도에 입력 오류가 없을 때만** `/result`로 이동한다. 오류가 있으면 이동하지 않고 화면 3의 차단 상태가 된다 |
| `미래지출 다시 입력하기` | 같은 URL에서 1단계로 되돌아가고 입력한 항목이 그대로 남는다. 단계 이동 오류 문구도 함께 초기화된다 |
...
| 연회비 한도 오류 상태에서 `결과 확인하기` | `결과를 확인할 수 없어요` / `카드 조건에 입력 오류가 있어요. 연회비 한도를 고치면 결과를 확인할 수 있습니다.` | `/result`로 **이동하지 않는다**(`preventDefault`). 링크에 `aria-disabled="true"`가 붙고 focus가 `annual-fee-cap`으로 이동하며 입력값·오류 문구는 유지된다. 값을 고치면 차단이 풀린다 |
...
- 2단계 차단은 **흐름 CTA에만** 걸린다. `/result` 직접 접근·새로고침은 spec §3.2대로 계속 열려
  대표 Fixture를 표시한다. 라우트를 잠그지 않는다.
- `aria-disabled`는 보조기기에 상태를 알리는 권고 속성이고 실제 차단은 `preventDefault`가 한다.
  두 장치를 함께 두어 색상 외 신호와 실제 동작이 어긋나지 않게 했다.
...
- `더 적게` 탭에서는 결론이 유지이므로 근거 1~3번이 현재 조합(예시 카드 A) 기준으로 표시되고,
  적용 조건 1번이 아래 문구로 바뀐다(렌더 원문 그대로다).

```
예시 카드 B의 생활 할인은 전월 실적 30만 원 이상일 때 적용됩니다. 이 시나리오의 예상 지출은 이 조건을 충족하지 않아, 변경 후보에 예시 카드 B를 더해도 연회비를 뺀 예상 순혜택이 현재 조합과 같습니다.
```

  적용 조건 2번(예시 카드 B의 온라인 쇼핑 할인)은 그대로 표시된다. `더 적게` 탭은 결론이 유지라
  근거 1·2번이 현재 조합(예시 카드 A)만 보여주므로 이 조건에 대응하는 영역은 화면에 없다 —
  변경 후보를 더해도 순혜택이 같은 이유를 설명하는 조건이며, 변경 후보 내역을 펼치는 화면은
  후속 체크포인트다.
...
위 표의 **아홉 상태 전부** `scrollWidth === clientWidth`이고, 같은 아홉 상태에서 문서의 가장
오른쪽까지 뻗은 요소의 `getBoundingClientRect().right`도 375로 뷰포트를 넘지 않는다.
...
| `결과 확인하기` (오류 없음) | `/result`로 이동 |
| `결과 확인하기` (연회비 한도 `-5`) | `url=/plan` 유지 · `결과를 확인할 수 없어요` 1건 · 차단 본문 1건 · `focus=annual-fee-cap` · 입력값 `-5` 유지 |
| 연회비 한도를 `12`로 고친 뒤 `결과 확인하기` | 차단 문구 0건 · `aria-disabled=false` · `/result` 이동 |
| `/result` 직접 접근 | 결과 제목 1건 (차단과 무관하게 열린다) |
