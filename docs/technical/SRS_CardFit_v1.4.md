...
| 원천 문서 | `SRS_CardFit_v1.3.md`(직전 기준선), `docs/product/PRD_CardFit_v1.2.md`(제품 기준선), `team-project_2nd/master-deck`, C-TEC-001~007 |

> 📘 **HTML 버전**: `docs/technical/SRS_CardFit_v1.4.html`

---

## 개요 (Summary)

CardFit은 **뱅크샐러드 내부에서** 기존 인증·마이데이터·보유카드·소비·상품 데이터 역량을 재사용하고, 미래 지출 계획을 기준으로 카드 조합을 설계하는 역기획 MVP다. 독립 MVP는 비식별 Fixture와 Platform Adapter로 전체 흐름을 실행하며, 실제 통합에서는 Adapter 구현체만 뱅크샐러드 내부 인터페이스로 교체한다. 본 SRS는 `docs/product/PRD_CardFit_v1.2.md`를 제품 기준선으로 사용하며 뱅크샐러드 운영 인프라가 본 MVP 스택과 같다고 가정하지 않는다.
...
CardFit v1.4는 뱅크샐러드 안에서 미래지출 기반 카드 조합 판단을 확장하는 역기획 SRS다. 독립 MVP는 Next.js App Router 단일 풀스택, Prisma, 로컬 Supabase CLI, 배포 Supabase, Tailwind CSS, shadcn/ui, Vercel AI SDK·Gemini, Vercel Git 배포를 의무 기준으로 사용한다. 실제 통합에서는 Mock Adapter만 내부 Identity·MyData·Card Catalog·Analytics Adapter로 교체하며 뱅크샐러드 운영 인프라를 이 스택으로 변경한다는 의미는 아니다.
...
- 원천 PRD: `docs/product/PRD_CardFit_v1.2.md`
- 변경 실행 계획: `docs/plans/PLAN_SRS_CardFit_v1.4_MVP_Stack_Alignment.md`
