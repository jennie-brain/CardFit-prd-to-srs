...
| 원천 문서 | `docs/product/PRD_CardFit_v1.0.md` (본 SRS는 이 PRD를 요구사항 도출의 유일한 정보 원천으로 한다) |

> 📘 **HTML 버전**: `docs/technical/SRS_CardFit_v1.0.html`

---

## 개요 (Summary)

CardFit은 소비 구조가 곧 바뀔 사용자가 과거 소비가 아닌 **미래 지출 계획**을 기준으로 보유·신규 카드 조합을 다시 계산받고, 계산 근거를 스스로 검증해 결제 포트폴리오를 결정하게 하는 서비스다. 본 SRS는 `docs/product/PRD_CardFit_v1.0.md`를 유일한 정보 원천으로 삼아, 예시 SRS 문서(AD-Core-Platform)가 취한 7섹션 포맷을 기본 구조로 채택하고, 그 포맷으로 담기지 않는 PRD 내용(가정·제약·의존성, 검증 계획, 참고자료)만 ISO/IEC/IEEE 29148:2018 표준 조항을 근거로 3개 챕터를 추가해 확장했다.
...
![CardFit Use Case Diagram](../product/diagrams/usecase_diagram_cardfit_v0.1.png)

> 이미지 파일: `docs/product/diagrams/usecase_diagram_cardfit_v0.1.png` (벡터 원본: `docs/product/diagrams/usecase_diagram_cardfit_v0.1.svg`)
...
- 원천 PRD: `docs/product/PRD_CardFit_v1.0.md`
