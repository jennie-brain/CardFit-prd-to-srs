...
- 변경 이력:
  ...
  - v1.0 정합성 보정 — 원본 Requirement Trace와 decision log에서 확정됐으나 통합 과정에서 누락된 **적게·예상대로·많이 3개 시나리오별 독립 카드 조합 추천** 요구사항을 복원. 원본에서 미정인 증감 폭과 계산 기간은 D5로 유지
...
| G6 예측이 틀려도 답을 준다 | 지출 시나리오 **3개(적게·예상대로·많이)를 모두 계산**하고 탭으로 제공한다. 첫 화면은 항상 **예상대로** 결론을 기본 노출한다 |
...
| 탐색 | 보조 시나리오 탭 열람률 | 결과 화면 도달자 중 `적게` 또는 `많이` 탭을 1회 이상 연 인원 수 ÷ 결과 화면 도달 인원 수 | 베타 1주차 실측으로 확정 | 관찰 지표 — 목표값 없음 | `scenario_tab_viewed` 이벤트 로그 | 주간 |
...
- **AC1**: Given 마이데이터 연동을 완료하고 보유카드가 1장 이상이며 미래지출을 1건 이상 입력한 사용자가, When `POST /calculate`를 요청하면, Then **p95 응답시간 ≤ 5초** 이내에 `적게`·`예상대로`·`많이` 3개 시나리오가 모두 산출되고 **예상대로 탭의 결론이 기본 노출**된다.
...
- **AC5**: Given 3개 시나리오 계산이 성공한 경우, When 사용자가 `적게`·`예상대로`·`많이` 탭을 전환하면, Then **추가 서버 재계산 없이** 사전 산출된 각 시나리오의 독립 카드 조합 또는 유지 결론이 표시되고, 각 탭에는 적용한 지출 가정이 누락 없이 표시된다.
- **AC6 (실패 케이스)**: Given 3개 시나리오 중 하나라도 필수 데이터 누락 또는 계산 실패가 발생한 경우, When 결과를 반환하려 하면, Then 전체 계산을 `부분` 상태로 처리하고 **어느 탭의 추천 결과도 노출하지 않는다**.
...
| **F-03** | 3개 지출 시나리오 동시 계산 — `적게`·`예상대로`·`많이` 각각에 실적구간·한도·연회비를 반영한 순혜택 산출 | A | **Must** | 대안(뱅크샐러드)은 후보 카드 1장과 비교. 3개 시나리오는 미래지출 예측 오차를 흡수하는 장치 | F-01, F-02, BenefitRule 수집·버전관리 체계, 시나리오 증감 폭(D5) 확정 |
| **F-04** | 시나리오별 독립 조합 최적화 + Net Benefit 게이팅 — `예상대로` 기본 노출, `적게`·`많이` 탭 전환 | A·B | **Must** | 경쟁자 체인에 없는 단계 — 각 지출 수준에서 "유지"를 정답으로 반환하는 유일한 설계 | F-03, Net Benefit 임계값(D2) 확정 |
...
| Calculation | 입력 스냅샷, `scenario`(`LOW`·`EXPECTED`·`HIGH`), 예상 총지출액, 계산 기간, 적용 rule_version 목록, 기준일, 미반영 항목 | 규칙 엔진 |
...
| `POST /calculate` | 입력: 미래지출·제약조건 / 출력: 3개 시나리오와 시나리오별 조합안 또는 유지 결론. 3개를 함께 반환하며 p95 ≤ 5s. 미래 입력 0건이면 400 |
...
| 계산(Calculation) | 요청 → (성공/실패/부분) | `LOW`·`EXPECTED`·`HIGH` 중 하나라도 필수 데이터 누락 또는 계산 실패이면 전체를 부분으로 처리하고 세 탭 모두 추천 노출 중단 |
...
| In | Out |
| --- | --- |
| F-01 미래지출 입력 · F-02 마이데이터 연동 · **F-03 적게/예상대로/많이 3개 시나리오 동시 계산** · **F-04 시나리오별 독립 조합 최적화와 탭 제공** · F-05 결제수단 배분 · F-06 근거 공개 · F-07 단계적 전환 제안 · F-08 이벤트 비종속 입력 · F-09 소득·지출 범위 입력 · F-11 초기값 제안 · F-12 스코프 고지 · F-13 완주 계측 | F-10 정기 재진단 알림 · F-14 카드 20장+ 대량 처리 최적화 · 해지·전환 실행 대행·상담·만류 대응 안내(직권·대행 권한 없음) · 신규카드 자동 발급 · 자동결제 · 대출·BNPL · 리텐션 전용 기능 · **시나리오 신뢰도/강건성 등급 배지(팀 미합의로 보류)** |
...
| 설계 | `적게`·`많이` 시나리오의 증감 폭과 계산 기간이 미확정이면 구현자마다 다른 결과를 산출 | D5에서 증감 폭·적용 방식·기간·총지출 집계식을 확정하고 회귀 테스트 기준 데이터셋으로 고정 | 결정 필요 — 개발 착수 전 |
...
| ADR-006 | 한 번의 `POST /calculate` 요청에서 동일한 마이데이터·미래지출 스냅샷으로 3개 시나리오를 모두 계산하고, 화면은 `예상대로` 탭을 기본 노출한다 | 보조 시나리오 때문에 마이데이터 호출을 늘리지 않고, 첫 화면에서 세 대안을 선택해야 하는 부담을 방지한다 | 확정 |
...
| A7 | `적게`·`많이` 보조 시나리오가 미래지출 예측 오차를 흡수하고 의사결정을 돕는다 | 2순위 | F-03·F-04의 3개 시나리오 설계 | 보조 탭 열람률·E2 인터뷰 |
...
- 시나리오 계산 정책(D5) 확정 — 계산 기간, `적게`·`많이` 증감 폭, 카테고리별 적용 방식, 예상 총지출 집계식

### 개발 착수 전 결정사항

| ID | 결정 필요 항목 | 현재 상태 | 기한 |
| :---: | --- | --- | :---: |
| D2 | Net Benefit 게이팅 임계값과 세부 비용 산식 | 미정 | 개발 착수 전 |
| D5 | 계산 기간(월·분기·연 등), `적게`·`많이`의 증감 폭, 증감률의 전체/카테고리별 적용 방식, 예상 미래 총지출액 집계식 | 미정 — 원본에서도 증감 폭은 열린 결정사항 | 개발 착수 전 |

> D5가 확정되기 전에는 3개 시나리오의 **구조와 UX만 확정 요구사항**이며, 구체 금액을 산출하는 공식은 확정 요구사항으로 간주하지 않는다. 임의의 ±비율을 제품 기본값으로 구현하지 않는다.
...
| E6 시나리오 정합성 | 3개 시나리오가 한 번에 일관되게 산출되고 예측 오차 탐색을 돕는다 | 동일 입력 스냅샷으로 `LOW`·`EXPECTED`·`HIGH` 결과와 탭 캡션을 검증하고, E2 참여자의 보조 탭 열람·이해도를 관찰 | 3개 결과 동시 산출 성공률 100% · 탭 전환 추가 계산 호출 0건 · 가정 캡션 누락 0건 · 부분 결과 노출 0건 |
...
| 3개 시나리오가 예측 오차를 흡수하면서 일관된 결과를 제공한다(G6·A7) | E6 시나리오 정합성 테스트 + E2 보조 탭 관찰 | 3개 동시 산출 성공률, 추가 계산 호출 건수, 캡션 누락 건수, 부분 결과 노출 건수, 보조 탭 열람률 |
